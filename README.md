...
The current documentation refers to the perspective of an **Test Bed Administrator**.
...
```
git clone -o seed_ca https://nsimatco@webgate.ec.europa.eu/CITnet/stash/scm/itb/docs-itb-ca.git ta
git remote add origin https://nsimatco@webgate.ec.europa.eu/CITnet/stash/scm/itb/docs-itb-ta.git
...
common documentation then these are made in the "ca" repository and then they can be synchronised to the current one. This
synchronisation is best made on a branch off "ta"'s master in order to review all changes. Any customisations needed for the "ta"
can be made in this temporary branch before it is merged onto "ta"'s master. Note that for sharing and building purposes the "ta"'s
